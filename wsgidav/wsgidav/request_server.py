...
#            isnewfile = not provider.exists(mappedpath)
    ...
    def doREPORT(self, environ, start_response):
        #TODO: support Depth: 1 and Depth: infinity
        
        path = environ["PATH_INFO"]
        res = self._davProvider.getResourceInst(path, environ)

        # RFC: By default, the REPORT method without a Depth header assumes 0
        ...
        if requestEL is None:   
            self._fail(HTTP_BAD_REQUEST, "requestEL is None")
        
        # addressbook-multiget is defined in RFC6352:8.7
        if requestEL.tag == "{urn:ietf:params:xml:ns:carddav}addressbook-multiget":
            propNameList = []   # list of proporties the client wants to know
            resList = []    # list of resources the client asks about
            for childNode in requestEL:
                if childNode.tag == "{DAV:}prop":
            ...
            self._fail(HTTP_BAD_REQUEST, "Only addressbook-multiget REPORT is supported.")
...
#        return util.sendMultiStatusResponse(environ, start_response, multistatusEL)
    ...
    def doOPTIONS(self, environ, start_response):
        ...
        if res and res.isCollection:
            # Existing collection
            #headers.append( ("Allow", "OPTIONS, HEAD, GET, PROPFIND, PROPPATCH, COPY, MOVE, LOCK, UNLOCK, REPORT") )
            headers.append( ("Allow", "DELETE, HEAD, GET, MKCOL, MOVE, OPTIONS, PROPFIND, PROPPATCH, PUT, REPORT") )
        elif res:
            # Existing resource
